...
const NavBar = () => {
  ...
  const [hasImageError, setHasImageError] = useState(false);
  const [isUpdate, setIsUpdate] = useState(false);
  const [gravatarUrl, setGravatarUrl] = useState<string>(""); // הוספתי את המצב הזה
  const userEmail = getUserEmailByToken();
  const userName = getUserNameByToken();
  const token = sessionStorage.getItem('token');

  useEffect(() => {
    // אם יש אימייל, תחשב את כתובת הגראוואטר
    if (userEmail) {
      setGravatarUrl(`https://www.gravatar.com/avatar/${md5(userEmail.trim().toLowerCase())}?s=200&d=identicon`);
    }
  }, [userEmail]); // האפקט ירוץ רק אם המייל משתנה
          ...
          <Box sx={{ flexGrow: 0 }}>
            <IconButton onClick={(e) => setAnchorElUser(e.currentTarget)} sx={{ p: 0 }}>
              <Avatar
                ...
                onError={() => setHasImageError(true)} // אם התמונה לא נטענת
              >
                {hasImageError && userName ? userName.charAt(0).toUpperCase() : ""}
              </Avatar>
            </IconButton>
